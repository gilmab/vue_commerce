...
import Prod from './Compo/Prod.vue'
import { useForm, router } from '@inertiajs/vue3';

// Filter for brands and category 
                            ...
                            <DisclosurePanel class="pt-6">
                              <div class="space-y-6">
                                <div v-for="brand in brands" :key="brand.id" class="flex items-center">
                                  <input :id="`filter-mobile-${brand.id}`" :value="brand.id" type="checkbox"  class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                  <label :for="`filter-mobile-${brand.id}`" class="ml-3 min-w-0 flex-1 text-gray-500">{{ brand.name }}</label>
                                </div>
                              </div>
                            </DisclosurePanel>
                          </Disclosure>
                ...
                <section aria-labelledby="products-heading" class="pb-24 pt-6">
                  ...
                  <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                    ...
                    <form class="hidden lg:block">
                      ...
                      <div class="flex items-center justify-between space-x-3">
                             <div class="basis-1/3">
                                  ...
                                  <input type="number" id="filters-price-from"
                                  placeholder="Min price" v-model="filterPrices.prices[0]" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-smtext-gray-900"/>
                             ...
                             <div class="basis-1/3">
                              ...
                              <input type="number"  id="filters-price-from"
                              placeholder="Min price" v-model="filterPrices.prices[1]" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-smtext-gray-900"/>

                            </div>

                            <SecondaryButton class="self-end"  
                            @click="priceFilter()"
                            >
                            ok 
                            </SecondaryButton>
                      ...
                      <Disclosure as="div"  class="border-b border-gray-200 py-6" v-slot="{ open }">
                        ...
                        <DisclosurePanel class="pt-6">
                          <div class="space-y-4">
                            <div v-for="brand in brands" :key="brand.id" class="flex items-center">
                              <input :id="`filter-${brand.id}`" :name="`${brand.id}`" :value="brand.id" type="checkbox" v-model="selectedBrands" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                        ...
                        <DisclosurePanel class="pt-6">
                          <div class="space-y-4">
                            <div v-for="category in categories" :key="category.id" class="flex items-center">
                              <input :id="`filter-${category.id}`" :name="`${category.id}`" :value="category.id" type="checkbox" v-model="selectedCategories"
                    ...
                    <div class="lg:col-span-3">
                      ...
                      <!-- Your content -->
                      <Prod :products="products.data">  </Prod>
